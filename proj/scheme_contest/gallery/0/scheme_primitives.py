...
@primitive("stream?")
def scheme_streamp(x):
    return x is nil or (scheme_pairp(x) and scheme_promisep(x.second))
...
def scheme_stream_cdr(x):
    check_type(x, scheme_streamp, 0, 'promise')
...
@primitive("rectangle")
def tscheme_rectangle(s1, s2):
    """Draws a rectangle along the current orientation, turtle on the lower left hand corner"""
    turtle.pendown()
    for i in range(2):
        turtle.forward(s1)
        turtle.left(90)
...
@primitive("dis-ani")
def tscheme_disable_animation():
    """Gets rid of that annoying screen refresh turtle does after every update. For speed."""
    turtle.tracer(0, 0)
    
@primitive("ud", "update")
def tscheme_update():
    """Manually updates the screen. Used when animations are disabled."""
    turtle.update()
    
@primitive("fillcolor")
def tscheme_fillcolor(color):
    turtle.fillcolor(color)
    
@primitive("pencolor")
def tscheme_pencolor(color):
    turtle.pencolor(color)
    
@primitive("dist", "distance")
def scheme_distance(x1, y1, x2, y2):
