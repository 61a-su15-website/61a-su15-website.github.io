...
class Promise(object):
    ...
    def __repr__(self):
        return "Promise({0}, {1})".format(repr(self.first), repr(self.second))
    ...
    def evaluate(self):
        if self.forced:
            return self.result
        self.forced = True
        self.result = scheme_eval(self.expr, env)
        return self.result
...
@primitive("cadr")
def scheme_car(x):
    check_type(x, scheme_pairp, 0, 'cadr')
    if x.second != nil:
        return x.second.first
    raise SchemeError

@primitive("fillcolor")
def scheme_fillcolor(x):
    turtle.fillcolor(x)
